...
function showPossibleCases(currentPos, steps){
  ...
  while(q.length){
    ...
    // cercle principal
    if(pos<48) q.push({pos:(pos+1)%48, rem:rem-1});

    // branches
    board.branches.forEach(branch=>{
      const idx = branch.indexOf(pos);
      if(idx!==-1 && idx+1<branch.length) q.push({pos:branch[idx+1], rem:rem-1});
    });
  }
...
function startTimer(sec){
  ...
  const t=$('timer'); t.style.display='block';
  let remaining=sec; t.textContent=remaining+"s";
  timer=setInterval(()=>{
    remaining--;
