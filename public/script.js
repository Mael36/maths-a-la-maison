...
function createActionCards(){
  ...
  actions.forEach(a=>{
    ...
    c.textContent=a;
    c.dataset.action=a;
    c.onclick=()=>{ socket.emit('choiceAction',{code:room,action:a}); };
    grid.appendChild(c);
  });
}
...
function startTimer(sec){
  ...
  timer=setInterval(()=>{
    ...
    if(sec<=0){
      clearInterval(timer); t.style.display='none';
...
socket.on("actionDrawn",data=>{ document.querySelectorAll('.actionCard').forEach(c=>c.style.transform="scale(1)"); document.querySelectorAll('.actionCard').forEach(c=>{ if(c.dataset.action===data.action) c.style.transform="scale(1.2)"; }); });
...
socket.on("results",data=>{ clearInterval(timer); $('timer').style.display="none"; $('resultText').textContent=data.correct?"Bonne réponse":"Mauvaise réponse"; $('resultText').style.color=data.correct?"#2e7d32":"#c62828"; $('resultBox').style.display='block'; updateScoreTable(data.players); setTimeout(()=>{$('questionBox').style.display='none';$('resultBox').style.display='none';},2500); });
