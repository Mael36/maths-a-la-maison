...
function updateScoreTable(players){
  ...
  players.forEach(p=>{
    ...
    line.textContent=`${p.name}: ${p.score} pts`; line.dataset.id=p.id; line.style.cursor='pointer';
...
/* ---------------- Socket events ---------------- */
socket.on('created', c=>{ room=c; showGame(); });
socket.on('joined', c=>{ room=c; showGame(); });
