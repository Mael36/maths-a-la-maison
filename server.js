...
// Board simple
const BOARD = { positions: Array.from({length:BOARD_LENGTH}, (_,i)=>({x: (i%8)*12.5, y: Math.floor(i/8)*12.5})) };
...
// Définition des actions et leur effet
const ACTIONS = [
  { name:'Flash', type:'timer', duration:30 },
...
io.on('connection', socket=>{
  ...
  socket.on('moveTo', ({code,pos})=>{
    ...
    const duration = action.type==='timer'?30:60;
